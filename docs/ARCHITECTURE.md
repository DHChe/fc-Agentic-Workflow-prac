...
- 출력: **구조화 출력**(`output_config.format`, JSON 스키마)으로 아래 형태를 강제한다. 스트림의 최종 메시지에서 JSON을 꺼내고(정확한 헬퍼는 13절), 받은 뒤에도 zod로 한 번 더 검증한다.
...
- 중복 배지와 삭제 → 중복 해제, 빈 대시보드, 암호 PDF, 모바일 업로드(아이폰에서 고른 사진이 JPG로 넘어오는지 포함), 다크 모드 배지, 시연 로그인 버튼, 랜딩, 시연 리허설(USER_FLOWS.md 9.1)은 수동 확인.
...
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | Clerk (브라우저용 키) | 개발·운영 인스턴스 분리 |
...
| ADR-16 | 테마는 기기 설정을 따르고 전환 버튼 없음 | 라이트/다크 토글(v1) | 확인할 화면이 절반. shadcn 기본 토큰이 양쪽 테마를 지원 |
